...
public class OrderController {
    ...
    */
    ...
    public Result delivery(@PathVariable String orderNo,
        ...
        orderService.delivery(orderNo, deliveryAddress, expressCompany, expressNo);
        return Result.success();
    }
    //确认收货
    ...
    */
    ...
    public Result getOrderAnalytics(@PathVariable Integer userId,
                                    @RequestParam(required = false) Integer year,
                                    @RequestParam(required = false) Integer month) {
        try {
            OrderAnalyticsDTO analytics = orderService.getOrderAnalytics(userId, year, month);
            return Result.success(analytics);
        } catch (Exception e) {
            return Result.error(Constants.CODE_500, "获取订单分析数据失败: " + e.getMessage());
        }
    }
}
